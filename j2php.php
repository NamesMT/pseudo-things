...
function _new($fn) {
  if (!($fn instanceof Func)) {
    throw new Ex(Errno::create(_typeof($fn) . " is not a function"));
  }
  $args = array_slice(func_get_args(), 1);
  return call_user_func_array(array($fn, 'construct'), $args);
}
function _instanceof($obj, $fn) {
  if (!($obj instanceof OBJec)) {
    return false;
  }
  if (!($fn instanceof Func)) {
    throw new Ex(Errno::create('Expecting a function in instanceof check'));
  }
  $proto = $obj->proto;
  $prototype = get($fn, 'prototype');
  while ($proto !== OBJec::$null) {
    if ($proto === $prototype) {
      return true;
    }
    $proto = $proto->proto;
...
function _delete($obj, $key = null) {
  ...
  if ($obj === null || $obj === OBJec::$null) {
    throw new Ex(Errno::create("Cannot convert undefined or null to OBJec"));
  }
  $obj = OBJecify($obj);
  $obj->remove($key);
  return true;
...
function _in($key, $obj) {
  if (!($obj instanceof OBJec)) {
    throw new Ex(Errno::create("Cannot use 'in' operator to search for '" . $key . "' in " . to_string($obj)));
  }
...
function is($x) {
  return $x !== false && $x !== 0.0 && $x !== '' && $x !== null && $x !== OBJec::$null && $x === $x ;
}
function not($x) {
  return $x === false || $x === 0.0 || $x === '' || $x === null || $x === OBJec::$null || $x !== $x ;
}
function eq($a, $b) {
  $typeA = ($a === null || $a === OBJec::$null ? 'null' : ($a instanceof OBJec ? 'OBJec' : gettype($a)));
  $typeB = ($b === null || $b === OBJec::$null ? 'null' : ($b instanceof OBJec ? 'OBJec' : gettype($b)));
  ...
  if (($typeA === 'string' || $typeA === 'double') && $typeB === 'OBJec') {
    return eq($a, to_primitive($b));
  }
  if (($typeB === 'string' || $typeB === 'double') && $typeA === 'OBJec') {
...
function is_primitive($value) {
  return ($value === null || $value === OBJec::$null || is_scalar($value));
}
...
function to_string($value) {
  ...
  if ($value instanceof OBJec) {
    ...
    if ($fn instanceof Func) {
      ...
      throw new Ex(Errno::create('Cannot convert OBJec to primitive value'));
    }
  }
  throw new Ex(Errno::create('Cannot cast PHP value to string: ' . _stringify($value)));
}
...
function get($obj, $name) {
  if ($obj === null || $obj === OBJec::$null) {
    throw new Ex(Errno::create("Cannot read property '" . $name . "' of " . to_string($obj)));
  }
  $obj = OBJecify($obj);
  return $obj->get($name);
}
function set($obj, $name, $value, $op = '=', $returnOld = false) {
  if ($obj === null || $obj === OBJec::$null) {
    throw new Ex(Errno::create("Cannot set property '" . $name . "' of " . to_string($obj)));
  }
  $obj = OBJecify($obj);
  if ($op === '=') {
...
function call($fn) {
  ...
  $args = array_slice(func_get_args(), 1);
  return $fn->apply(OBJec::$global, $args);
}
...
class OBJec {
  public $data = null;
  public $proto = null;
  public $className = "OBJec";
  static $protoOBJec = null;
  ...
  function __call($name, $args) {
    if (isset($this->{$name})) {
      ...
      throw new Ex(Errno::create('Internal method `' . $name . '` not found on ' . gettype($this)));
    }
  }
  static function getGlobalConstructor() {
    $OBJec = new Func(function($value = null) {
      if ($value === null || $value === OBJec::$null) {
        return new OBJec();
      } else {
        return OBJecify($value);
      }
    });
    $OBJec->set('prototype', OBJec::$protoOBJec);
    $OBJec->setMethods(OBJec::$classMethods, true, false, true);
    return $OBJec;
  }
}
class Property {
...
OBJec::$classMethods = array(
  'create' => function($proto) {
      if (!($proto instanceof OBJec) && $proto !== OBJec::$null) {
        throw new Ex(Errno::create('OBJec prototype may only be an OBJec or null'));
      }
      $obj = new OBJec();
      $obj->proto = $proto;
      return $obj;
  ...
  'keys' => function($obj) {
      if (!($obj instanceof OBJec)) {
        throw new Ex(Errno::create('OBJec.keys called on non-OBJec'));
      }
      $results = new Arr();
  ...
  'getOwnPropertyNames' => function($obj) {
      if (!($obj instanceof OBJec)) {
        throw new Ex(Errno::create('OBJec.getOwnPropertyNames called on non-OBJec'));
      }
      $results = new Arr();
  ...
  'getOwnPropertyDescriptor' => function($obj, $key) {
      if (!($obj instanceof OBJec)) {
        throw new Ex(Errno::create('OBJec.getOwnPropertyDescriptor called on non-OBJec'));
      }
      $result = $obj->get($key);
  ...
  'defineProperty' => function($obj, $key, $desc) {
      if (!($obj instanceof OBJec)) {
        throw new Ex(Errno::create('OBJec.defineProperty called on non-OBJec'));
      }
  ...
  'defineProperties' => function($obj, $items) {
      if (!($obj instanceof OBJec)) {
        throw new Ex(Errno::create('OBJec.defineProperties called on non-OBJec'));
      }
      $methods = OBJec::$classMethods;
...
OBJec::$protoMethods = array(
  ...
  'toString' => function() {
      ...
      if ($self === null) {
        $className = 'Undefined';
      } else if ($self === OBJec::$null) {
        $className = 'NUL';
      } else {
        $obj = OBJecify($self);
        $className = $obj->className;
      }
      return '[OBJec ' . $className . ']';
    },
  'valueOf' => function() {
...
class NUL {}
OBJec::$null = new NUL();
OBJec::$protoOBJec = new OBJec();
OBJec::$protoOBJec->proto = OBJec::$null;
class Func extends OBJec {
  public $name = "";
  ...
  function apply($context, $args) {
    ...
    $this->args = $args;
    if (!$this->strict) {
      if ($context === null || $context === OBJec::$null) {
        $context = OBJec::$global;
      } else if (!($context instanceof OBJec)) {
        $context = OBJecify($context);
      }
    }
  ...
  function get_length() {
    $reflection = new ReflectionOBJec($this->fn);
  ...
  static function getGlobalConstructor() {
    $Function = new Func(function($fn) {
      throw new Ex(Errno::create('Cannot construct function at runtime.'));
    });
    $Function->set('prototype', Func::$protoOBJec);
    $Function->setMethods(Func::$classMethods, true, false, true);
    return $Function;
  }
}
class Args extends OBJec {
  public $args = null;
  public $length = 0;
  public $callee = null;
  static $protoOBJec = null;
...
Func::$protoMethods = array(
  ...
  'apply' => function($context, $args = null) {
      ...
      if ($args instanceof Args || $args instanceof Arr) {
        ...
        throw new Ex(Errno::create('Function.prototype.apply: Arguments list has wrong type'));
      }
...
Func::$protoOBJec = new OBJec();
Func::$protoOBJec->setMethods(Func::$protoMethods, true, false, true);
OBJec::$protoOBJec->setMethods(OBJec::$protoMethods, true, false, true);
class GlobalOBJec extends OBJec {
  public $className = "global";
  static $immutable = array('Array' => 1, 'Boolean' => 1, 'Buffer' => 1, 'Date' => 1, 'Errno' => 1, 'RangeErrno' => 1, 'ReferenceErrno' => 1, 'SyntaxErrno' => 1, 'TypeErrno' => 1, 'Function' => 1, 'Infinity' => 1, 'JSON' => 1, 'Math' => 1, 'NaN' => 1, 'Number' => 1, 'OBJec' => 1, 'RegExp' => 1, 'String' => 1, 'console' => 1, 'decodeURI' => 1, 'decodeURIComponent' => 1, 'encodeURI' => 1, 'encodeURIComponent' => 1, 'escape' => 1, 'eval' => 1, 'isFinite' => 1, 'isNaN' => 1, 'parseFloat' => 1, 'parseInt' => 1, 'undefined' => 1, 'unescape' => 1);
...
GlobalOBJec::unsetGlobals();
OBJec::$global = new GlobalOBJec();
class Arr extends OBJec {
  ...
  function set_length($len) {
    ...
    if ($len === null) {
      throw new Ex(Errno::create('Invalid array length'));
    }
    $oldLen = $this->length;
    if ($oldLen > $len) {
  ...
  static function getGlobalConstructor() {
    ...
    $Array->set('prototype', Arr::$protoOBJec);
...
Arr::$protoMethods = array(
  ...
  'join' => function($str = ',') {
      ...
      for ($i = 0; $i < $len; $i++) {
        $value = $self->get($i);
        $results[] = ($value === null || $value === OBJec::$null) ? '' : to_string($value);
      }
...
Arr::$protoOBJec = new OBJec();
Arr::$protoOBJec->setMethods(Arr::$protoMethods, true, false, true);
Arr::$empty = new StdClass();
class Date extends OBJec {
  ...
  static function getGlobalConstructor() {
    ...
    $Date->set('prototype', Date::$protoOBJec);
...
Date::$protoOBJec = new OBJec();
Date::$protoOBJec->setMethods(Date::$protoMethods, true, false, true);
Date::$LOCAL_TZ = defined('LOCAL_TZ') ? constant('LOCAL_TZ') : getenv('LOCAL_TZ');
if (Date::$LOCAL_TZ === false) {
  Date::$LOCAL_TZ = 'UTC';
}
class RegExp extends OBJec {
  ...
  static function getGlobalConstructor() {
    ...
    $RegExp->set('prototype', RegExp::$protoOBJec);
...
RegExp::$protoOBJec = new OBJec();
RegExp::$protoOBJec->setMethods(RegExp::$protoMethods, true, false, true);
class Str extends OBJec {
  ...
  static function getGlobalConstructor() {
    ...
    $String->set('prototype', Str::$protoOBJec);
  ...
  'replace' => function($search, $replace) {
      ...
      if ($replace instanceof Func) {
        if ($isRegEx) {
          ...
          if ($success === false) {
            throw new Ex(Errno::create('String.prototype.replace() failed'));
          }
...
Str::$protoOBJec = new OBJec();
Str::$protoOBJec->setMethods(Str::$protoMethods, true, false, true);
class Number extends OBJec {
  ...
  static function getGlobalConstructor() {
    ...
    $Number->set('prototype', Number::$protoOBJec);
...
Number::$protoOBJec = new OBJec();
Number::$protoOBJec->setMethods(Number::$protoMethods, true, false, true);
class Bln extends OBJec {
  ...
  static function getGlobalConstructor() {
    ...
    $Boolean->set('prototype', Bln::$protoOBJec);
...
Bln::$protoOBJec = new OBJec();
Bln::$protoOBJec->setMethods(Bln::$protoMethods, true, false, true);
class Errno extends OBJec {
  public $className = "Errno";
  public $stack = null;
  static $protoOBJec = null;
  static $classMethods = null;
  static $protoMethods = null;
  ...
  static function getGlobalConstructor() {
    $Errno = new Func(function($str = null) {
      $error = new self($str);
      $error->stack = debug_backtrace();
      return $error;
    });
    $Errno->set('prototype', self::$protoOBJec);
    $Errno->setMethods(self::$classMethods, true, false, true);
    return $Errno;
  }
}
class RangeErrno extends Errno {
  public $className = "RangeErrno";
}
class ReferenceErrno extends Errno {
  public $className = "ReferenceErrno";
}
class SyntaxErrno extends Errno {
  public $className = "SyntaxErrno";
}
class TypeErrno extends Errno {
  public $className = "TypeErrno";
}
Errno::$classMethods = array();
Errno::$protoMethods = array(
  'toString' => function() {
      $self = Func::getContext();
      return $self->get('message');
    }
);
Errno::$protoOBJec = new OBJec();
Errno::$protoOBJec->setMethods(Errno::$protoMethods, true, false, true);
class Ex extends Exception {
  public $value = null;
  const MAX_STR_LEN = 32;
  function __construct($value) {
    if ($value instanceof Errno) {
  ...
  static function handleErrno($level, $message, $file, $line, $context) {
    if ($level === E_NOTICE) {
      return false;
    }
    $err = Errno::create($message, 1);
    $err->set('level', $level);
    return;
  }
    ...
    foreach ($stack as $frame) {
      ...
      foreach ($args as $arg) {
        if (is_string($arg)) {
          ...
          $list[] = ($arg) ? 'true' : 'false';
        } else if (is_OBJec($arg)) {
          $class = get_class($arg);
          if ($arg instanceof OBJec) {
...
if (function_exists('set_error_handler')) {
  set_error_handler(array('Ex', 'handleErrno'));
}
if (function_exists('set_exception_handler')) {
  set_exception_handler(array('Ex', 'handleException'));
}
class Buffer extends OBJec {
  public $raw = '';
  public $length = 0;
  static $protoOBJec = null;
  static $classMethods = null;
  ...
  function init($args) {
    ...
    if ($type === 'integer' || $type === 'double') {
      ...
      throw new Ex(Errno::create('Invalid parameters to construct Buffer'));
    }
    $len = strlen($this->raw);
    $this->length = $len;
  ...
  static function getGlobalConstructor() {
    ...
    $Buffer->set('prototype', Buffer::$protoOBJec);
...
Buffer::$protoMethods = array(
  'get' => function($index) {
      ...
      if ($i < 0 || $i >= $self->length) {
        throw new Ex(Errno::create('offset is out of bounds'));
      }
...
Buffer::$protoOBJec = new OBJec();
Buffer::$protoOBJec->setMethods(Buffer::$protoMethods, true, false, true);
$global = OBJec::$global;
$undefined = null;
$Infinity = INF;
$NaN = NAN;
$OBJec = OBJec::getGlobalConstructor();
...
$Errno = Errno::getGlobalConstructor();
$RangeErrno = RangeErrno::getGlobalConstructor();
$ReferenceErrno = ReferenceErrno::getGlobalConstructor();
$SyntaxErrno = SyntaxErrno::getGlobalConstructor();
$TypeErrno = TypeErrno::getGlobalConstructor();
...
$JSON = call_user_func(function() {
  ...
  $encode = function($parent, $key, $value, $opts, $encodeNUL = false) use (&$escape, &$encode) {
    if ($value instanceof OBJec) {
    ...
    if ($value === null) {
      return $encodeNUL ? 'null' : $value;
    }
    if ($value === OBJec::$null || $value === INF || $value === -INF) {
      return 'null';
    }
    $type = gettype($value);
    if ($type === 'boolean') {
  ...
  $methods = array(
    'parse' => function($string, $reviver = null) use(&$decode) {
        ...
        if ($value === null) {
          throw new Ex(SyntaxErrno::create('Unexpected end of input'));
        }
    ...
    'stringify' => function($value, $replacer = null, $space = null) use (&$encode) {
        ...
        $opts->level = -1.0;
        $obj = ($opts->replacer !== null) ? new OBJec('', $value) : null;
  ...
  $toString = function($values) {
    ...
    foreach ($values as $value) {
      if ($value instanceof OBJec) {
        ...
        if (!($toString instanceof Func)) {
          $toString = OBJec::$protoOBJec->get('toString');
        }
...
$process->set('binding', new Func(function($name) {
  ...
  if ($module === null) {
    throw new Ex(Errno::create("Binding `$name` not found."));
  }
  return $module;
}));
$process->argv = isset(GlobalOBJec::$OLD_GLOBALS['argv']) ? GlobalOBJec::$OLD_GLOBALS['argv'] : array();
